...
{
    public class SpecDataConvert
    {
        private int pixelNumber = 0;
        private float[] waveLength = null; //像素对应的波长
        private double FACTOR_VOL_TO_INTEG = 4.096 / 65536.0;//电压积分转换系数

        #region 单子模式

        private SpecDataConvert()
        {
        }
        private static object syncObj = new object();
        private static SpecDataConvert instance = null;

        public static SpecDataConvert GetInstance()
        {
            lock (syncObj)
            {
                if (instance == null)
                {
                    instance = new SpecDataConvert();
                }
            }
            return instance;
        }
        ...
        public float GetWaveByPixel(int pixel)
        {
            if (waveLength == null)
            {
                throw new Exception("NoWavelengh");
            }
